...
export async function POST(req: NextRequest) {
  ...
  if (grant_type !== "authorization_code") return corsJson({ error: "unsupported_grant_type" }, { status: 400 });
  if (!code || !redirect_uri || !clientId || !codeVerifier) return corsJson({ error: "invalid_request" }, { status: 400 });
